setwd("D:/Spring 2018/STA/Project/newcode/expt1/")
...
ggplot(data, aes(x=type, y=improvement, group=1)) +
  ...
  annotate("text", label =  paste("Min Improvement-", data$type[grep(min(data$improvement[2:nrow(data)][1]), data$improvement)], " :", 
                                  round(min(data$improvement[2:nrow(data)]),2), sep = ""),
           x = data$type[grep(min(data$improvement[2:nrow(data)]), data$improvement)[1]], 
           y = min(data$improvement[2:nrow(data)])-2, color = "black") + 
  ggtitle("NN Improvement summarization on 10% outliers on single feature") + theme(plot.title = element_text(hjust = 0.5))+
  annotate("rect", xmin = "NN25", xmax = "NN59", ymin = 17, ymax = 24,
...
ggplot(plot_data, aes(x = Type, y = purity, group = 1)) + 
  ...
  ggtitle("Purity comparison. Outlier - 10% on single feature")+ theme(plot.title = element_text(hjust = 0.5))+
